###############################################################################
#'
#' main_code_lineage/model_code_main.R
...
# Helper nimbleFunction to track the log-likelihood of the full model
...
#### calcIntensity function for efficiently handling iNat IPP sum 
...
joint_model_SVCs_asRanefs <- nimbleCode({
  ...
  for (i in 1:nLamBeta) {
    ...
    # indicator variables for lineage component
    lin_gamma[i] ~ dbern(p_incl_lin)
    ...
    CAR_effect[1:nSpatCell, i] ~ dcar_normal(adj = adj[1:adj_len],
                                             ...
                                             zero_mean = 1)
    for (j in 1:nLineage) {
      lin_effect[j, i] ~ dnorm(0, 1)
    }
    lin_effect[nLineage + 1, i] <- 0 # Add in extra cat. for cells w/o lineage that are in the spatial area (for now)
    for (j in 1:nEcoregion) {
      eco_effect[j, i] ~ dnorm(0, 1)
    }
    eco_effect[nEcoregion + 1, i] <- 0
    
    # Prior on psi mean/constant effect, on spatial variance when incl.
    sigma_CAR_0[i] ~ dinvgamma(shape = 0.5, scale = 0.5)
    sigma_lin_0[i] ~ dinvgamma(shape = 0.5, scale = 0.5)
    sigma_eco_0[i] ~ dinvgamma(shape = 0.5, scale = 0.5)
    
    # Actual spatial sigma
    sigma_CAR[i] <- sigma_CAR_0[i] * CAR_gamma[i] + cc
    sigma_lin[i] <- sigma_lin_0[i] * lin_gamma[i] + cc
    ...
    for (cell in 1:nSpatCell) {
      lambda_beta[cell, i] <- 
        ...
        sigma_lin[i] * lin_effect[lineage[cell], i] +
