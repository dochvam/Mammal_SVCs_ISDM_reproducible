...
make_lineage_interaction_dat <- function(covs_df, occ_covars,
                                         lineage_final,
                                         lineage_levels) {
  
  covs_df <- left_join(covs_df, lineage_final, by = c("spec_range_gridID"))
  
  lineage_vector <- factor(covs_df$target_lineage, levels = lineage_levels)
  contrasts(lineage_vector) <- contr.sum
  # Levels are first thru. (n-1)th. -1s are the final lineage
  model_mtx <- as.matrix(model.matrix(~lineage_vector)[, -1])
  
  lineage_int_groups <- rep(1:length(occ_covars), each = ncol(model_mtx))
  lineage_interaction_data <- matrix(ncol = length(lineage_int_groups), 
                                     nrow = nrow(covs_df))
  
  lineage_int_names <- paste0(
    expand.grid(occ_covars, 1:(ncol(model_mtx)))[,1], "_L",
    expand.grid(occ_covars, 1:(ncol(model_mtx)))[,2]
  )
  
  for (i in 1:length(occ_covars)) {
    inds <- which(lineage_int_groups == i)
    
    this_cov_mtx <- matrix(rep(unlist(covs_df[, occ_covars[i]]),
                               ncol(model_mtx)),
                           nrow = nrow(model_mtx), ncol = ncol(model_mtx))
    
    lineage_interaction_data[, inds] <- model_mtx * this_cov_mtx
  }
  return(list(
    df = lineage_interaction_data,
    lineage_vector = lineage_vector,
    lineage_int_groups = lineage_int_groups
  ))
}
...
get_inits <- function(modtype, occ_covars, det_covars, spatial_model,
                      this_adj_info = NULL, nLineage, nEcoregion, lineage_datlist,
  ...
  if (spatial_model == "SVCs") {
    inits_list <- list(
      ...
      lin_effect = matrix(rnorm(nLineage*length(occ_covars), sd = 0.5),
      ...
      CAR_gamma = rbinom(length(occ_covars), size = 1, prob = 0.2),
      lin_gamma = rbinom(length(occ_covars), size = 1, prob = 0.5),
      eco_gamma = rbinom(length(occ_covars), size = 1, prob = 0.5),
      
      sigma_CAR_0 = rep(0.25, length(occ_covars)),
      sigma_lin_0 = rep(0.25, length(occ_covars)),
      sigma_eco_0 = rep(0.25, length(occ_covars)),
      
      sigma_CAR = rep(0.25, length(occ_covars)),
      sigma_lin = rep(0.25, length(occ_covars)),
...
get_monitors <- function(modtype, inat_disttype, spatial_model) {
  ...
  if (inat_disttype == "NB") {
    monitors <- c(monitors, "overdisp_inat"
                  )
  }
  ...
  if (spatial_model == "CAR") {
    ...
    if (modtype == "separate") {
      monitors2 <- c(monitors2, "inat_spat_ranef"
                     )
    }
  } else {
    monitors2 <- "dummy"
...
fit_integrated_model_with_CV <- function(nfolds, 
  ...
  outfile <- paste0("intermediate/integration_results/final_", 
                    species, "_", modtype, "_", spatial_model, suffix, "_lineage_samples.RDS")
  if (!overwrite && file.exists(outfile)) {
    writeLines(paste0("Not running ", species, " suffix ", suffix,
                      " at time ", Sys.time()), 
               con = "log.txt")
    return()
  }
  ...
  start_time <- Sys.time()
  # I disable k-fold validation by setting nfolds to 0. I still construct
  ...
  lambda_covar_indices <- c(grep("lambda_beta", param_inds), 
                            ...
                            grep("lin_gamma", param_inds),
                            grep("sigma_CAR", param_inds),
                            grep("sigma_eco", param_inds),
                            grep("sigma_lin", param_inds))
